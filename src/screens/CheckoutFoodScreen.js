...
const CheckoutFood = ({ navigation }: Props) => {
  ...
  const _onPlaceOrder = () => {
    ...
    if (payMethod === 'online') {
      setLoading(true)
      checkoutAPI({ food_orders: true, ids: cart_id, user_address_id: selectedAddress.id, cod: false }, openWebViewer, getError)
    } else if (payMethod === 'cod') {
      setLoading(true)
      checkoutAPI({ food_orders: true, ids: cart_id, user_address_id: selectedAddress.id, cod: true }, placeSuccess ,getError)
    } else {
