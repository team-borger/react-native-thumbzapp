...
import { deleteFoodCartAPI } from '../../services/products';
...
const MyComponent = (props, {bulkDeleteTrigger, choosenItems}) => {
  ...
  const itemSelected = (product_id, checked) => {
    ...
    const selected = cartItems.find((item) => item.food_id == product_id)
    if(checked) {
      tempCart.push(selectedItems)
    }
    else {
      let tempIndex = tempCart.findIndex((item) => item.food_id == product_id)
  ...
  const bulkDelete = () => {
    ...
    selectedItems.forEach((markedItem) => {
      const tempIndex = tempCart.findIndex((item) => item.food_id == markedItem.food_id)
      tempCart.splice(tempIndex, 1);
  ...
  const quantityChanged = (x) => {
    let tempCart = cartItems;
    const tempIndex = tempCart.findIndex((item) => item.food_id == x.product_id)
